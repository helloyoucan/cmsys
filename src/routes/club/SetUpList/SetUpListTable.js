...
import SetUpListForm from './SetUpListForm';
import SetUpListModal from './SetUpListModal';

@connect(state => ({
  setUpList: state.setUpList,
}))
export default class SetUpListTable extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    modalLoading: false,
  ...
  handleDelete(delOneId) {
     ...
     * delOneId：删除单个时的传参
     * */
    const {dispatch, setUpList: {data: {pagination}}} = this.props;
  ...
  render() {
    const {setUpList: {loading: userLoading, data}} = this.props;
    ...
    return (
      <PageHeaderLayout>
        ...
        <SetUpListModal modalVisible={this.state.modalVisible}
