...
export default class ActivityListTable extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    modalLoading: false,
  ...
  handelModal(key, id) {
    switch (key) {
      case 'add':
        this.props.dispatch({
          type: 'activityList/goToPath',
          payload: {
            path: '/clubManagement/clubApproval/alPage',
          }
        })
        /*this.setState({
         modalVisible: true,
  ...
  startProcess(type, id) {
    confirm({
      ...
      onOk: () => {
        ...
        const that = this;
        this.props.dispatch({
          type: 'activityList/startProcess',
          payload: {
            id: id
          },
          callback: (res) => {
            const pagination = that.props.activityList.data.pagination;
            const params = {
              keyword: formValues.keyword,
  ...
  handleDelete(delOneId) {
     ...
     * delOneId：删除单个时的传参
     * */
    const {dispatch, activityList: {data: {pagination}}} = this.props;
    ...
    confirm({
      ...
      onOk: () => {
        dispatch({
          type: 'activityList/changeLoading',
          payload: {
            bool: true,
          },
        });
        dispatch({
          type: 'activityList/del',
          payload: {
            id: delOneId
          },
          callback: () => {
  ...
  render() {
      ...
      },
      {
        ...
        render: (val, row) => {
          ...
          return ( <div>
            ...
            <Link to={{pathname: '/clubManagement/clubApproval/alPage', data: {id: val}}}> 查看详细</Link>
    ...
    return (
      <PageHeaderLayout>
        ...
        {/*<ActivityListModal modalVisible={this.state.modalVisible}
